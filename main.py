...
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///ogbhs.db'  
db = SQLAlchemy(app) 
...
def AddBaseAdmin():
    ...
    rooms = [Rooms(id=1, floor=0, roomtype="D/B NON AC Rooms", description="Double Bed", status=st, ghId=0, pricePerDay=800, occupancy=2, ac=0),
                Rooms(id=2, floor=0, roomtype="D/B AC Rooms", description="Double Bed", status=st, ghId=0, pricePerDay=1000, occupancy=2, ac=1),
                Rooms(id=3, floor=1, roomtype="Single NON AC Rooms", description="Single Bed", status=st, ghId=0, pricePerDay=2000, occupancy=1, ac=0),
                Rooms(id=4, floor=2, roomtype="Single AC Rooms", description="Single Bed", status=st, ghId=0, pricePerDay=5000, occupancy=1, ac=1),
    ...
    for i in amenities:
        db.session.add(i)
        db.session.commit()
    admin = User(id=0, firstname="keerthana",lastname="vuriti", email="knk.seproject@gmail.com",username="keerthana", password="keerthana@123", address="Bobbili , Andhra pradesh", age=19, gender="Female", rollstd="GH001",usertype="Admin")
...
def show_rooms():
    ...
    if checkindate is not None and checkoutdate is not None:
        ...
        if datetime.now() <= checkInDate <= checkOutDate and (checkOutDate-datetime.now()).days < 190:
            if (checkOutDate.year - checkInDate.year) * 12 + (checkOutDate.month - checkInDate.month) > 6:
                raise ValueError("Stay duration cannot exceed 6 months.")
            pass
        else:
